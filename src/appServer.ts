...
import { getNeonSlots } from './db/neon.js';
import {
  ensurePaymentsTable,
  getNeonSlotById,
  markSlotOccupied,
...
// Make sure the DB connection/checks have run before handling any
// request. initDb() is cached, so this is a no-op after the first
// call on a given process (or warm serverless instance).
app.use(async (req, res, next) => {
  try {
    await initDb();
    next();
  } catch (error) {
    res.status(503).json({ success: false, reason: 'Database is not available' });
  }
});
...
// GET so it can be called by Vercel Cron (or hit manually / from a
// browser) as a belt-and-braces sweep for expired bookings. The main
// path is the opportunistic completeExpiredNeonBookings() calls above
// (on every /api/slots and /api/bookings/my request), which is what
// actually keeps things fresh between cron runs — see vercel.json,
// where Hobby-plan cron can only run once/day.
app.get('/api/cron/expire-bookings', async (req, res) => {
  try {
    const count = await completeExpiredNeonBookings();
    res.json({ success: true, expired: count });
  } catch (error) {
    console.error('❌ Cron expiry sweep failed:', error);
    res.status(500).json({ success: false, reason: 'Expiry sweep failed' });
  }
});
...
// ------------------------------------------------------------------
// One-time DB startup checks (connection test + inspection logging +
// creating the payments table if it doesn't exist yet). Cached so it
// only actually runs once per warm process/serverless instance, no
// matter how many times initDb() is called.
// ------------------------------------------------------------------
let dbReadyPromise: Promise<void> | null = null;

export function initDb(): Promise<void> {
  if (!dbReadyPromise) {
    dbReadyPromise = testNeonConnection()
      ...
      .catch((error) => {
        ...
        // Reset so the next request/invocation can retry instead of
